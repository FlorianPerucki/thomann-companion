...
(function () {
  ...
  function fromAdPage() {
    ...
    if (!leaves.length) return [{ key: 'lbc:' + id, title, priceEl: null, priceValue: null, currency: 'EUR', mount: h1 }];
    ...
    return leaves
      ...
      .map((el, i) => ({ key: 'lbc:' + id + (el === primary ? '' : ':' + i), title, priceEl: el, priceValue: value.value, currency: 'EUR', mount: el }));
  }
